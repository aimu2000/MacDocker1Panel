...
check_script_update() {
    ...
    if [[ -n "$remote_content" ]]; then
        # 先从注释中提取版本（格式: # 版本: 20260208V2.2）
        # 注释通常更新更及时
        local remote_version=$(echo "$remote_content" | grep -E "^# 版本:.*[0-9]{8}V[0-9]+\.[0-9]+" | head -1 | grep -oE "[0-9]{8}V[0-9]+\.[0-9]+" | head -1)
        
        # 如果注释没有，尝试从变量提取（格式: CURRENT_VERSION="20260208V2.2"）
        if [[ -z "$remote_version" ]]; then
            remote_version=$(echo "$remote_content" | grep -E "^CURRENT_VERSION=\"[^\"]*\"" | head -1 | cut -d'=' -f2 | tr -d '"' | tr -d ' ')
        fi
...
update_script() {
    ...
    # 获取脚本绝对路径
    local script_path="$(cd "$(dirname "$0")" 2>/dev/null && pwd)/$(basename "$0" 2>/dev/null)"
    if [[ "$script_path" == "/" ]]; then
        script_path="$0"
    fi
    
    # 备份当前脚本
    # 备份文件名格式: mac1panel-20260209-V2.1-备份.sh
    local backup_file="$HOME/mac1panel-${CURRENT_VERSION}-backup-$(date +%Y%m%d%H%M%S).sh"
    cp "$script_path" "$backup_file" 2>/dev/null || cp "$0" "$backup_file" 2>/dev/null || {
        error "无法备份脚本，请确保脚本路径正确"
        return 1
    }
    ...
    if curl --connect-timeout 10 --max-time 30 -fsSL "$SCRIPT_URL" -o "$temp_script"; then
        ...
        if bash -n "$temp_script" 2>/dev/null; then
            ...
            # 替换当前脚本
            cp "$temp_script" "$script_path" 2>/dev/null || mv "$temp_script" "$script_path" 2>/dev/null || {
                error "无法更新脚本，请手动操作: mv $temp_script $script_path"
                return 1
            }
